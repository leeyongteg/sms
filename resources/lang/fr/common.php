<?php
return [
"school_name" => "Nom de l'école",

"school_code" => "Code scolaire",

"dashboard" => "Tableau de bord",

"copy" => "Copier",

"year" => "Année",

"sl" => "SL",

"average_mark" => "Marque moyenne",

"date_of_publication_of_result" => "Date de publication du résultat",

"edit" => "édition",

"view" => "Afficher",

"close" => "Fermer",

"error" => "Erreur",

"url" => "URL",

"setup" => "Configuration",

"absent" => "Absent",

"actions" => "Actions",

"created_by" => "Créé par",

"sms" => "SMS",

"cancelled" => "Annulé",

"details" => "Détails",

"approved" => "Approuvé",

"image" => "Image",

"male" => "Homme",

"female" => "Femme",

"other" => "Autres",

"others" => "Autres",

"attached_file" => "Joindre un fichier",

"timezone" => "Fuseau horaire",

"notes" => "Notes",

"enable" => "Activer",

"disable" => "Désactiver",

"saas" => "Saas",

"pending" => "En attente",

"branch" => "Direction générale",

"waiting" => "En attente",

"closed" => "Fermé",

"topic" => "Rubrique",

"document" => "Document",

"home" => "Accueil",

"module_version" => "Version du module",

"purchase_code" => "Code d'achat",

"installed_domain" => "Domaine installé",

"system_activated_date" => "Date d'activation du système",

"check_update" => "Vérifier le point",

"view_note" => "Afficher la note",

"class_name" => "Nom de classe",

"student_list" => "Liste des étudiants",

"select_month" => "Sélectionner un mois",

"national_id_card" => "Carte d'identité nationale",

"national_id_number" => "Carte d'identité nationale",

"delete_custom_field" => "Supprimer une zone personnalisée",

"birth_certificate_number" => "Numéro de certificat de naissance",

"email_address" => "Adresse électronique",

"academic_year" => "Année universitaire",

"class_sec" => "Classe (section)",

"language" => "Langue",

"select_language" => "Sélectionner une langue",

"select_class" => "Sélectionner une classe",

"select_subject" => "Sélectionner le sujet",

"select_subjects" => "Sélectionner des sujets",

"select_teacher" => "Sélectionner l'enseignant",

"select_item" => "Sélectionner un article",

"select_exam" => "Sélectionner l'extension",

"select_type" => "Sélectionner un type",

"select_lesson" => "Sélectionner la leçon",

"select_academic_year" => "Sélectionner une année universitaire",

"select_student" => "Sélectionner l'étudiant",

"select_exam_type" => "Sélectionner le type d'extension",

"select_width" => "Sélectionner la largeur",

"select_one" => "Sélectionner un",

"select_institution" => "Sélectionner une institution",

"phone_no" => "Numéro de téléphone",

"reference_no" => "N ° de référence",

"info" => "Infos",

"attach_file" => "Joindre un fichier",

"optional" => "Facultatif",

"option" => "Option",

"mobile_no" => "Non mobile",

"add_timeline" => "Ajouter une chronologie",

"add_information" => "Ajouter des informations",

"father_name" => "Nom du père",

"save_information" => "Sauvegarder les informations",

"update_information" => "Sauvegarder les informations",

"search_by_name" => "Rechercher par nom",

"are_you_sure_to_approve" => "Are You Sure To Approuver",

"result_archive" => "Archive de résultats",

"days" => "Jours",

"label" => "Libellé",

"mark" => "Marquer",

"weekend" => "Weekend",

"reference_jpeg_png" => "Référence (jpeg/png)",

"date_time" => "Date / heure",

"route" => "Achemée",

"add_menu" => "Ajouter un menu",

"delete_menu" => "Supprimer le menu",

"parents_list" => "Liste des parents",

"my_children" => "Mes enfants",

"teacher_list" => "Liste des enseignants",

"calendar" => "Calendrier",

"view_as_school_admin" => "Afficher en tant qu'administrateur scolaire",

"view_as_sass_admin" => "Afficher en tant qu'administrateur Saas",

"not_required" => "Non requis",

"phone_number" => "Numéro de téléphone",

"website" => "Site Web",

"meeting_duration" => "Durée de la réunion",

"meeting_time" => "Heure de la réunion",

"meeting_start_before" => "Début de la réunion avant",

"edit_meeting" => "Editer la réunion",

"add_meeting" => "Ajouter une réunion",

"meeting_list" => "Liste de réunion",

"join" => "Rejoindre",

"virtual_class_details" => "Détails de la classe virtuelle",

"start_date_time" => "Date / heure de début",

"meeting_id" => "ID de réunion",

"participants" => "Participants",

"start_join" => "Démarrer / Joindre",

"min" => "Min",

"icon" => "Icône",

"cancel_the_record" => "Annuler l'enregistrement",

"meeting_start_join_before" => "Début / jointure de la réunion avant",

"the_session_is_required" => "La session est requise",

"virtual_class" => "Classe virtuelle",

"virtual_meeting" => "Réunion virtuelle",

"created_at" => "Créé à",

"host_id" => "ID hôte",

"meeting_room" => "Salle de réunion",

"meetings_reports" => "Rapport de réunion",

"member_type" => "Type de membre",

"to_date" => "Jusqu'à présent",

"from_date" => "Date de début",

"virtual_class_reports" => "Rapports de classe virtuelle",

"not_yet_start" => "Pas encore de début",

"class_details" => "Détails de la classe",

"class_id" => "ID classe",

"date_of_meeting" => "Date de la réunion",

"start_join_before" => "Démarrer / rejoindre avant",

"virtual_class_list" => "Liste de classes virtuelles",

"teachers" => "Enseignants",

"logout" => "Déconnexion",

"view_profile" => "Afficher le profil",

"profile_update" => "Mettre à jour profil",

"edit_virtual_class" => "Editer la classe virtuelle",

"add_virtual_class" => "Ajouter une classe virtuelle",

"select_group" => "Sélectionner un groupe",

"you_have" => "Vous avez",

"add_staff" => "Ajouter un personnel",

"personal_info" => "Informations personnelles",

"subscription" => "Abonnement",

"institution" => "Institution",

"institution_list" => "Liste des institutions",

"saas_dashboard" => "Tableau de bord Saas",

"trial_institutions" => "Institutions de première instance",

"package" => "Module",

"add_module" => "Ajouter un module",

"day_list" => "Liste des jours",

"add_school" => "Ajouter une école",

"edit_school" => "Editer l'école",

"update_school" => "Mettre à jour École",

"delete_school" => "Supprimer l'école",

"save_school" => "Sauvegarder l'école",

"session" => "Session",

"add_session" => "Ajouter une session",

"edit_session" => "Editer la session",

"update_session" => "Session mise à jour",

"delete_session" => "Supprimer une session",

"save_session" => "Sauvegarder l'école",

"opening_date" => "Date d'ouverture",

"school_domain" => "Domaine scolaire",

"select_package" => "Sélectionner un package",

"purchase_date" => "Date d'achat",

"expaire_date" => "Date d'expiration",

"infix_edu" => "M. Lee",

"infix_clasified" => "Infix classifié",

"date_|_day" => "Date | Jour",

"purchase_list" => "Liste d'achats",

"welcome_to" => "Bienvenue dans",

"welcome_to_administrator" => "Bienvenue dans Administration",

"are_you_sure_to_detete_this_item" => "Etes-vous sûr de supprimer cet article",

"welcome" => "Bienvenue",

"module_chat" => "Module",

"add_news" => "Ajouter des informations",

"monday" => "Lundi",

"tuesday" => "Mardi:",

"wednesday" => "Mercredi",

"thursday" => "Jeudi:",

"friday" => "Vendredi",

"Saturday" => "Samedi",

"sunday" => "Dimanche",

"parents_phone_number" => "Numéro de téléphone des parents",

"select_module" => "Sélectionner un module",

"view_as_saas_admin" => "Afficher en tant qu'administrateur Saas",

"quick_search" => "Recherche rapide",

"add_days" => "Ajouter des jours",

"select_style" => "Sélectionner un style",

"view_question" => "Afficher la question",

"add_new_module" => "Ajouter un nouveau module",

"add_payment" => "Ajouter un paiement",

"view_payment" => "Afficher le paiement",

"Add payment" => "Afficher le paiement",

"if_unpaid_admission_will_be_cancelled_after" => "En cas de non-paiement, l'admission sera annulée après",

"edit_receive_details" => "Editer les détails de réception",

"payroll" => "Retenues",

"menu_manage" => "Gérer les menus",

"view_bank_payment" => "Afficher le paiement bancaire",

"delete_bank_payment" => "Supprimer le paiement bancaire",

"email_template" => "Modèle de courrier électronique",

"view_payment_bank" => "Afficher la banque de paiement",

"delete_payment_bank" => "Supprimer une banque de paiement",

"key_search" => "Rechercher",

"leave_group" => "Groupe de congés",

"delete_group" => "Supprimer le groupe",

"add_new" => "Ajouter nouveau",

"add_in_wallet" => "Ajouter dans le portefeuille",

"add_fees_payment" => "Ajouter un paiement de taxes",

"school_signup" => "Signup scolaire",

"text_search" => "Rechercher des personnes ou des groupes",

"chat_files" => "Fichiers",

"search_list" => "Liste de recherche",

"uploaded" => "Téléchargement",

"add_note" => "Ajouter une note",

"failed" => "Ces données d'identification ne correspondent pas à nos enregistrements.",

"throttle" => "Trop de tentatives de connexion. Veuillez réessayer en: secondes secondes.",

"Create" => "Créer",

"Title" => "Titre",

"Description" => "Description",

"Status" => "Statut",

"Image" => "Image",

"SL" => "SL",

"Dashboard" => "Tableau de bord",

"Enable" => "Activer",

"Disable" => "Désactiver",

"Duration" => "Durée",

"Browse" => "Parcourir",

"Update" => "Mise à jour",

"ID" => "ID",

"Password" => "Mot de passe",

"Topic" => "Rubrique",

"Actions" => "Actions",

"Add" => "Ajouter",

"Class" => "Classe",

"Yes" => "Oui",

"Closed" => "Fermé",

"No" => "Non",

"Save" => "Sauvegarder",

"Date" => "Date",

"Time" => "Heure",

"Start" => "Démarrer",

"Join" => "Rejoindre",

"Select" => "Sélectionner",

"View" => "Afficher",

"Edit" => "Editer",

"Delete" => "Supprimer",

"Cancel" => "Annuler",

"Name" => "Nom",

"Details" => "Détails",

"Close" => "Fermer",

"Signature" => "Signature",

"create" => "Créer",

"browse" => "Parcourir",

"photo" => "Photo",

"name" => "Nom",

"update" => "Mise à jour",

"file" => "Fichier",

"new" => "Nouveau",

"message" => "Message",

"type" => "Type",

"delete" => "Supprimer",

"yes" => "Oui",

"select" => "Sélectionner",

"no" => "Non",

"user" => "Utilisateur",

"notification" => "Notification",

"group" => "Groupe",

"mark_as_read_only" => "Marquer comme lecture seule",

"teacher" => "Enseignant",

"student" => "Étudiant",

"people" => "Personnes",

"start" => "Démarrer",

"to" => "Vers",

"add" => "Ajouter",

"user_role" => "Rôle utilisateur",

"member" => "Membre",

"media_files" => "Fichiers de support",

"search" => "Rechercher",

"your_voice_is_recording" => "Votre voix est un enregistrement",

"mark_all_as_read" => "Marquer comme lu",

"chat_list" => "Liste de discussion",

"description" => "Description",

"search_result_of" => "Rechercher le résultat de",

"Subjects" => "Sujets",

"Logout" => "Déconnexion",

"Pending" => "En attente",

"Active" => "Actif",

"Login" => "Connexion",

"To" => "Vers",

"Action" => "Action",

"User" => "Utilisateur",

"Type" => "Type",

"Staff" => "Personnel",

"Email" => "Courrier électronique",

"Phone" => "Téléphone",

"URL" => "URL",

"Language" => "Langue",

"Address" => "Adresse",

"date" => "Date",

"Download" => "Télécharger",

"Role" => "Rôle",

"Failed" => "Echec",

"Reject" => "Rejeter",

"File" => "Fichier",

"Waiting" => "En attente",

"Info" => "Infos",

"Submit" => "Soumettre",

"Error" => "Erreur",

"New" => "Nouveau",

"Welcome" => "Bienvenue",

"Days" => "Jours",

"Copy" => "Copier",

"Print" => "Imprimer",

"Website" => "Site Web",

"All" => "Tous",

"Year" => "Année",

"status" => "Statut",

"active" => "Actif",

"email" => "Courrier électronique",

"time" => "Heure",

"id" => "ID",

"action" => "Action",

"None" => "Néant",

"TimeZone" => "Fuseau horaire",

"role" => "Rôle",

"submit" => "Soumettre",

"login" => "Connexion",

"subjects" => "Sujets",

"class" => "Classe",

"section" => "Section",

"subject" => "Objet",

"class_list" => "Liste des classes",

"room" => "Salle",

"save" => "Sauvegarder",

"of" => "De",

"select_all" => "Sélectionner tout",

"select_room" => "Sélectionner une salle",

"print" => "Imprimer",

"verify" => "Vérifier",

"from" => "De",

"reject" => "Rejeter",

"delete_item" => "Supprimer l'article",

"balance" => "Solde",

"select_criteria" => "Sélectionner les critères",

"select_status" => "Sélectionner le statut",

"no_data_available_in_table" => "Aucune donnée disponible dans la table",

"phone" => "Téléphone",

"are_you_sure_to_delete" => "Etes-vous sûr de supprimer cet élément?",

"are_you_sure_to_disable" => "Etes-vous sûr de désactiver cet élément?",

"are_you_sure_to_enable" => "Etes-vous sûr d'activer cet élément?",

"cancel" => "Annuler",

"address" => "Adresse",

"reference" => "Référence",

"download" => "Télécharger",

"note" => "Remarque",

"student_photo" => "Photo de l'élève",

"none" => "Non",

"select_section" => "Sélectionner une section",

"class_Sec" => "Classe (sec.)",

"date_of_birth" => "Date de naissance",

"gender" => "Sexe",

"mobile" => "Mobile",

"select_id_card" => "Sélectionner une carte d'ID",

"select_role" => "Sélectionner un rôle",

"blood_group" => "Groupe sanguin",

"all" => "Tous",

"class_section" => "Classe (sec.)",

"duration" => "Durée",

"approve" => "Approuver",

"roll_number" => "Numéro de rouleau",

"online" => "En ligne",

"menu" => "Menu",

"back" => "Précédent",

"student_name" => "Nom de l'étudiant",

"break" => "Pause",

"title" => "Titre",

"signature" => "Signature",

"staff" => "Personnel",

"school" => "École",

"login_details" => "Détails de connexion",

"enter_email_address" => "Entrer l'adresse électronique",

"enter_password" => "Entrez mot de passe",

"remember_me" => "Se souvenir de moi",

"forget_password" => "Mot de passe oublié",

"password" => "Mot de passe",

];